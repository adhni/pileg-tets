#!/usr/bin/env python3
"""Python port of the representation-gap analysis with threshold-aware defaults."""
...
def build_representation_outputs(
    ...
    return {
        "summary": summary,
        "winning_candidates": winning_candidate_rows,
        "coverage_rows": coverage_rows,
        "lowest_coverage_rows": coverage_rows_sorted[:20],
        "province_rows": province_rows,
        "party_distortion_rows": party_distortion_rows,
    }


def main() -> None:
    candidate_rows = read_csv(PREPARED_DATA_DIR / "dpr_candidates_standardized.csv")
    threshold_seat_rows = read_csv(PYTHON_OUTPUT_DIR / "dpr_estimated_winners" / "estimated_seats_by_district_party.csv")
    raw_seat_rows = read_csv(PYTHON_OUTPUT_DIR / "dpr_estimated_winners" / "estimated_seats_by_district_party_raw.csv")

    threshold_outputs = build_representation_outputs(candidate_rows, threshold_seat_rows, "threshold_adjusted")
    raw_outputs = build_representation_outputs(candidate_rows, raw_seat_rows, "raw_all_parties")

    summary = {
        **threshold_outputs["summary"],
        "default_model": "threshold_adjusted",
        "threshold_adjusted": threshold_outputs["summary"],
        "raw": raw_outputs["summary"],
    }
    ...
    write_csv(
        ...
        threshold_outputs["winning_candidates"],
    )
    write_csv(
        ...
        threshold_outputs["coverage_rows"],
    )
    write_csv(
        ...
        threshold_outputs["lowest_coverage_rows"],
    )
    write_csv(
        OUTPUT_DIR / "coverage_by_province.csv",
        ["province", "districts", "avg_coverage", "winner_model"],
        threshold_outputs["province_rows"],
    )
    write_csv(
        ...
        threshold_outputs["party_distortion_rows"],
    )

    write_csv(
        OUTPUT_DIR / "winning_candidates_with_shares_raw.csv",
        [
            "province",
            "district",
            "party_code",
        ...
        raw_outputs["winning_candidates"],
    )
    write_csv(
        OUTPUT_DIR / "coverage_by_district_raw.csv",
        [
            "province",
            "district",
            "district_total_votes",
        ...
        raw_outputs["coverage_rows"],
    )
    write_csv(
        OUTPUT_DIR / "lowest_coverage_districts_raw.csv",
        [
            "province",
            "district",
            "district_total_votes",
        ...
        raw_outputs["lowest_coverage_rows"],
    )
    write_csv(
        OUTPUT_DIR / "coverage_by_province_raw.csv",
        ["province", "districts", "avg_coverage", "winner_model"],
        raw_outputs["province_rows"],
    )
    write_csv(
        OUTPUT_DIR / "party_distortion_raw.csv",
        [
            "party_code",
            "party_name",
            "total_votes",
        ...
        raw_outputs["party_distortion_rows"],
    )
