...
- `python/representation_gap.py`
  Ports the core representation-gap calculations using explicit dapil seat counts and the threshold-adjusted DPR winner estimates by default.
...
- `python/report_builder.py`
  Builds one consolidated HTML/Markdown report from the Python outputs, including parliamentary-threshold effects and replacement winners.

- `python/build_interactive_dashboard.py`
  Builds a standalone interactive HTML dashboard with party-ratio logic, candidate drilldowns, threshold-aware DPR winner views, and district-level replacement-winner explanations.

- `python/build_pileg_seat_dashboard.py`
