...
DAPIL_SEATS_SOURCE = ROOT / "analysis" / "notebooks" / "pileg_seat copy.qmd"
...
def district_key(label: str) -> str:
    value = normalize_space(label).upper()
    tokens = value.split()
...
def extract_r_c_vector(text: str, label: str) -> str:
    marker = f"{label} = c("
    start = text.find(marker)
    if start < 0:
        raise ValueError(f"Could not find vector {label!r}")
    ...
    while idx < len(text) and depth > 0:
        ...
        idx += 1
    if depth != 0:
        raise ValueError(f"Unbalanced parentheses while reading vector {label!r}")
    return "".join(out)


def prepare_dapil_seats(district_to_province: Dict[str, str]) -> List[Dict[str, object]]:
    text = DAPIL_SEATS_SOURCE.read_text(encoding="utf-8")
    district_labels = re.findall(r'"([^"]+)"', extract_r_c_vector(text, "Daerah_Pemilihan"))
    seat_vector = extract_r_c_vector(text, "Jumlah_Kursi")
    seat_values = [int(token) for token in re.findall(r"\b\d+\b", re.sub(r"#.*", "", seat_vector))]
    if len(district_labels) != len(seat_values):
        raise ValueError("Dapil seat labels and values have different lengths")

    district_lookup = {district_key(district): district for district in district_to_province}
    prepared_rows: List[Dict[str, object]] = []

    for district_label, seat_count in zip(district_labels, seat_values):
        key = district_key(district_label)
        if key not in district_lookup:
            raise KeyError(f"Could not match district {district_label!r} to DPR data")
        district = district_lookup[key]
        prepared_rows.append(
            {
                ...
                "source_file": "analysis/notebooks/pileg_seat copy.qmd",
            }
        )
...
def write_data_dictionary(metadata: List[Dict[str, object]], notes: List[str]) -> None:
    lines = [
        ...
        "",
        "These files are the Python-first prep layer for the repo. The current R/Quarto notebooks remain untouched; the outputs below are normalized and ready for future Python analyses.",
        "",
        "## Files",
        "",
    ]
...
def main() -> None:
    ...
    metadata = [
        ...
        },
        {
            ...
            "source": "analysis/notebooks/pileg_seat copy.qmd.",
